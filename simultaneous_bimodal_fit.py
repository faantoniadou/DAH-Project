...
def plot_gaussian():
    ...
    gauss_peak_11 = gaus(peak_data, *pars_1[0])/area
    gauss_peak_12 = gaus(peak_data, *pars_2[0])/area
    gauss_peak_21 = gaus(peak_data, *pars_1[1])/area
    gauss_peak_22 = gaus(peak_data, *pars_2[1])/area
    gauss_peak_31 = gaus(peak_data, *pars_1[2])/area
    gauss_peak_32 = gaus(peak_data, *pars_2[2])/area


    
    #mean3 = peak_data[np.where(bimodal/area == np.max(bimodal/area))][0]
    #print(mean3)
    ...
    #gaus_data = gaus_data/area
    #clear_data = clear_data/area

    mean_1 = peak_data[np.abs(gaus_data/area - np.max(gauss_peak_11 + gauss_peak_12)).argmin()]
    mean_2 = peak_data[np.abs(gaus_data/area - np.max(gauss_peak_21 + gauss_peak_22)).argmin()]
    mean_3 = peak_data[np.abs(gaus_data/area - np.max(gauss_peak_31 + gauss_peak_32)).argmin()]
    
    print(mean_1)
    print(mean_2)
    print(mean_3)
    
    plt.vlines(x = mean_1, ymin = 0, ymax = np.max(gaus_data/area), ls='--', lw=0.75, colors = 'magenta', label = 'peak 1 : mean = '+str(np.round_(mean_1, 3)))
    ...
    pylab.plot(peak_data, gaus_data/area, 'k', label='best fit gaussians')      # these are normalised to 1 now
    pylab.plot(all_masses_trimmed, clear_data_trimmed/area, 'r--', label='signal data')
    pylab.ylim(0)
...
def residuals():
    ...
    # compute residuals 
    difference = clear_data/area - gaus_data/area
