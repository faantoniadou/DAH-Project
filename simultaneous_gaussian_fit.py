...
def residuals():
    ...
    clear_data = clear_data[np.where(gaus_data/np.sum(gaus_data) != 0)]

    maxima = maxima/np.sum(gaus_data)
    pylab.plot(peak_data, gaus_data/np.sum(gaus_data), 'm', label='best fit gaussians')      # these are normalised to 1 now
    pylab.plot(peak_data, clear_data/np.sum(gaus_data),'k-',linewidth=0.8, label='signal data')
    ...
    #Residual plot
    difference = gaus_data/np.sum(gaus_data) - clear_data/np.sum(gaus_data)
