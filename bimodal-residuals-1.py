...
def remove_bg():
    ...
    all_counts, all_masses = np.histogram(region1, bins=get_bins(bg_masses)-330, range=[np.min(region1), np.max(region1)])
...
def plot_bg():
    ...
    bg_masses, bg_counts, bg_all = background()
    all_counts, _ = np.histogram(region1, bins=get_bins(bg_masses)-330, range=[np.min(region1), np.max(region1)])
