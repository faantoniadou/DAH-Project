...
#%%
region1 =  np.array(xmass)[np.where((xmass > 10.18) & (xmass < 10.55))]
...
def background():
    ...
    side_low = region1[np.where((region1 < edge1) & (region1 > 10.18))]
    side_high = region1[np.where((region1 < 10.55) & (region1 > edge2))]
    empty_regions = np.zeros(len(region1[np.where((region1 >= edge1) & (region1 <= 10.18))]))
    ...
    bg_counts, bg_masses = np.histogram(bg_data, bins=get_bins(bg_data), range=[np.min(bg_data), np.max(bg_data)], density=False)
...
def remove_bg():
    ...
    all_counts, all_masses = np.histogram(region1, bins=get_bins(bg_all), range=[np.min(region1), np.max(region1)])
...
def plot_bg():
    ...
    bg_masses, bg_counts, bg_all = background()
    all_counts, _ = np.histogram(region1, bins=get_bins(bg_all), range=[np.min(region1), np.max(region1)])
    ...
    pylab.title("Graph of " + str(xmass_name) + " data for the third peak" )
    pylab.xlabel(xmass_name)
    pylab.ylabel(xmass_units)
    pylab.ylim(0)
    pylab.legend()
    plot_histogram(xmass_name, bg_all, xmass_units, num_bins=get_bins(bg_all))
